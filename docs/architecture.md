...
Layout lives behind an endpoint rather than in `app.js` because it is the one piece of geometry the server also needs, for the sample map. One implementation means a tidied map and a seeded map are laid out by the same code. The frontend posts to it after every structural edit (collapse, expand, add, delete, reparent, resize) as well as on an explicit tidy, and applies only the returned coordinates so an edit made while the request is in flight is not overwritten. The returned coordinates land on the model at once and the nodes and edges are interpolated to them over a quarter of a second, so a save or an undo during the motion carries the settled positions rather than a frame of it. `prefers-reduced-motion` skips the interpolation. The children of a collapsed node are laid out from their parent's top edge, so they carry sane coordinates until the expand that re-lays the tree.
...
- **Markdown** is rendered by marked with a renderer that highlights fenced code through highlight.js, turns a `mermaid` fence into a diagram container, slugs heading ids, and turns `[!TIP]`-style blockquotes into callouts. The output is sanitised by DOMPurify before it reaches the modal. The modal is opened before the post-parse steps run, because Mermaid measures text and a diagram drawn inside a closed dialog comes out with zero-sized labels. Then copy buttons are added to each code block, Mermaid draws the diagrams with a config read off the page palette, and Lucide draws the callout icons last. Links open in a new tab.
